...
function lwrte_image() {
	var self = this;
	var panel = self.create_panel('插入图片URL', 385);
	panel.append('\<input type="text" id="url" size="30" value="http://">\
				<p class="submit clearfix"><button type="button" id="ok">确定</button><button type="button" id="cancel">取消</button></p>'
).show();
...
function lwrte_link() {
	var self = this;
	var panel = self.create_panel("请输入选择文字的链接地址", 385);
	panel.append('\<input type="text" id="url" size="30" value="http://">\
				<p class="submit clearfix"><button type="button" id="ok">确定</button><button type="button" id="cancel">取消</button></p>').show();
	...
	*/
	...
	$('#ok', panel).click( 
		function() {
			...
			var tmp = $('<a></a>').append(self.get_selected_html());

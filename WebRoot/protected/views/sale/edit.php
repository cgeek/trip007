<div class="main">
	...
	<form action="/sale/save" method="POST">
		<input type="hidden" name="id" value="<?=isset($sale_db['pin_id']) ? $sale_db['pin_id'] :'';?>">
		<input type="hidden" name="cover_image_id" value="<?=isset($sale_db['cover_image']) ? $sale_db['cover_image'] :'';?>">

		<?php $is_edit = isset($sale_db) ? true : false ;?>
		<?php $is_gonglue = ((isset($_GET['type']) && $_GET['type'] ==2) || (isset($sale_db) && $sale_db['type'] == 2) ) ? true : false ;?>
		<input type="hidden" name="type" value="<?=$is_edit ? $sale_db['type'] : $is_gonglue ? 2:1;?>">
		<div class="pb-main">
...
<?php if(isset($sale_db['cover_image'])):?>
	<img src="<?=upimage($sale_db['cover_image'],'small');?>">
<?php endif;?>
					</div>
					<div class="cover_image_show_layout">
<?php if(isset($sale_db['cover_image'])):?>
	<img src="<?=upimage($sale_db['cover_image'],'big');?>">
<?php endif;?>
					</div>
				</div>
				<div class="pb-title-holder">
					<h3 class="pb-section-title">标题<span>(选填:30个字以内)</span></h3>
					<input type="text" class="pb-title-input" name="title" value="<?=isset($sale_db['title']) ? $sale_db['title'] :'';?>">
				</div>
				<div class="pb-desc-holder">
					<h3 class="pb-section-title">概述<span>(必填:120个字以内。如果同步到微博，同步此内容)</span></h3>
					<textarea name="desc" class="pb-desc-textarea"><?=isset($sale_db['desc']) ? $sale_db['desc'] :'';?></textarea>
				...
				<div class="pb-content-holder">
					...
					<div class="pb-content-editor">
						...
						<?=isset($sale_db['content']) ? stripslashes(htmlspecialchars_decode($sale_db['content'])) :'';?>
						</script>
					</div>
				</div>
			</div>
			<div class="pb-action-holder">
			<?php if(isset($sale_db)):?>
		...
		<div class="pb-aside">
			<div class="pb-aside-inbox">
				...
				<?php if(isset($sale_db) && $sale_db['status'] == 0):?>
				...
				<div id="post-tags-holder">
					<textarea name="tags" placeholder="添加关联目的地,用逗号隔开；如香港,泰国"><?=isset($sale_db['tags']) ? stripslashes(htmlspecialchars_decode($sale_db['tags'])) :'';?></textarea>
				</div>
			</div>
		</div>
	</form>

...
function InsertSmiley(url) {
    ...
    editor.execCommand('InsertHTML', span.innerHTML.replace(/<img/ig,"<img style='border:0px' "));
    dialog.close();
}
...
inforBox['tab0']=['Kiss','Love','Yeah','啊！','背扭','顶','抖胸','88','汗','瞌睡','鲁拉','拍砖','揉脸','生日快乐','大笑','瀑布汗~','惊讶','臭美','傻笑','抛媚眼','发怒','打酱油','俯卧撑','气愤','?','吻','怒','胜利','HI','KISS','不说','不要','扯花','大心','顶','大惊','飞吻','鬼脸','害羞','口水','狂哭','来','发财了', '吃西瓜', '套牢', '害羞', '庆祝', '我来了', '敲打', '晕了', '胜利', '臭美', '被打了', '贪吃', '迎接', '酷', '','','','','','','','','','','','','','','开心','偷笑','大哭','滴汗','叹气','超赞','??','飞吻','天使','撒花','生气','被砸','吓傻','随意吐'];
...
FaceHandler={
    ...
    imageReviewFolders:{tab0:'jx_review/',tab1:'tsj_review/',tab2:'ldw_review/',tab3:'bobo_review/',tab4:'babycat_review/',tab5:'face/',tab6:'youa_review/'},
...
function createTab(tabName){
	var faceVersion = "?v=1.1",//版本号
	    ...
	    imageReviewPath=sBasePath+FaceHandler.imageReviewFolders[tabName],
	...
	for(;i<imgNum;){
		...
		for(var j = 0;j<imgColNum;j++,i++){
			...
			if ( faceImage ){
				...
				sReviewUrl = imageReviewPath + faceImage ;
                ...
                textHTML.push('<img  style="background-position:left '+offset+'px;" title="'+infor+'" src="'+sBasePath+'default/0.gif" width="'+iWidth+'" height="'+iHeight+'"></img>');
                textHTML.push('</span>');
            }else{
